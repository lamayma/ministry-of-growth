/* ═══════════════════════════════════════════════════════════
   ...
   MOG-JS-001 / Rev. 3 — dynamic dashboard per plant
═══════════════════════════════════════════════════════════ */

'use strict';

/* ── Per-plant dashboard data ───────────────────────────── */
const plantMetrics = {
  '041': {
...
function updateDashboard(objNum) {
  ...
  data.cards.forEach((cfg, i) => {
    ...
    if (cfg.statusWord !== undefined) {
      ...
      if (valEl) {
        valEl.dataset.count = cfg.count;
        valEl.innerHTML = `0<span class="metric-unit">${cfg.unitText}</span>`;
      }

      const fill = card.querySelector('.metric-bar-fill');
      if (fill) {
        fill.style.cssText = `--target:${cfg.barTarget};--color:${cfg.barColor}`;
        fill.style.width = '0%';
      }
      ...
      if (fill) {
        setTimeout(() => { fill.style.width = cfg.barTarget; }, 60);
      }
    }
  });
}
...
function initTabs() {
  ...
  btns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Update dashboard for this plant
...
/* ── Metric bar + counter animations (initial load) ─────── */
